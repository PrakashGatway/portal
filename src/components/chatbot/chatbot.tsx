              ...
              <div className="flex min-h-0 flex-1 flex-col bg-[#fffaf6]">
                {selectedArticle && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="shrink-0 border-b border-[#eadfd9] bg-white px-4 py-3"
                  >
                    <div className="flex items-center gap-2">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#fff5f0] text-[#ff6040]">
                        <BookOpen size={14} />
                      </div>
                          ...
                          {isStreaming &&
                            ...
                            msg.role === "assistant" && (
                              <motion.span
                                ...
                                className="ml-1 inline-block h-3 w-0.5 rounded-full bg-[#ff704f]"
                              />
                            )}
                        </div>
                      </div>
                ...
                <div className="shrink-0 border-t border-[#eadfd9] bg-white p-3">
                  <div className="flex items-end gap-2 rounded-2xl border border-[#eadfd9] bg-[#fffaf6] p-1.5 transition focus-within:border-[#ff704f] focus-within:ring-4 focus-within:ring-[#ff704f]/10">
                    <textarea
                      value={message}
                      ...
                      rows={1}
                      className="max-h-24 min-h-[42px] flex-1 resize-none bg-transparent px-2.5 py-2.5 text-sm text-[#17243a] outline-none placeholder:text-[#8791a3]"
                    />
                    <motion.button
                      whileTap={{ scale: 0.9 }}
                      onClick={isStreaming ? stopGeneration : sendMessage}
                      disabled={!isStreaming && !message.trim()}
                      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-white transition disabled:cursor-not-allowed disabled:opacity-30 ${
                        isStreaming
                          ? "bg-[#17243a]"
