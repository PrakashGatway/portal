...
export const DynamicFormFields = ({ formData, setFormData, pageType }) => {
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto my-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <div className="p-5 grid grid-cols-1 gap-5 md:grid-cols-2">
          {fields.map((field) => (
            <div
              key={field.name}
              className={field.type === "editor" ? "md:col-span-2" : ""}
            >
              <DynamicField
                field={field}
                value={
                  formData.pageContent?.[field.name] || formData[field.name]
                }
      ...
      {sections && sections.length > 0 && (
        <div className="space-y-3">
          {sections.map((sectionConfig) => {
            const sectionIndex = formData.sections.findIndex(
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {sectionConfig.fields.some((f) => f.type === "array") && (
                      <span className="text-xs px-2 py-0.5 bg-gray-200 dark:bg-gray-700 rounded-full text-gray-600 dark:text-gray-400">
                        {Array.isArray(
                          section.content[
                            sectionConfig.fields.find((f) => f.type === "array")
                              ?.name
                          ],
                        )
                          ? section.content[
                              sectionConfig.fields.find(
                                (f) => f.type === "array",
                              ).name
                            ].length
                          : 0}{" "}
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`flex items-center gap-1.5 bg-white dark:bg-gray-700 px-2.5 py-1 rounded-md border border-gray-200 dark:border-gray-600 transition-opacity ${expanded ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                    >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          e.stopPropagation();
                          updateSectionOrder(sectionIndex, e.target.value);
                        }}
                      ...
                      {sectionConfig.fields.map((field) => (
                        <div key={field.name}>
                          <DynamicField
                            ...
                            onChange={(name, value) =>
                              updateSectionContent(sectionIndex, name, value)
                            }
                          />
                        </div>
                      ))}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2.5">
          {items.map((item, idx) => (
            <div
              key={idx}
              className="group bg-gray-50 dark:bg-gray-800/40 border border-gray-200 dark:border-gray-700 rounded-lg p-3.5"
            >
                  ...
                  <div key={subField.name}>
                    ...
                    {subField.type === "editor" ? (
                      ...
                      <textarea
                        value={item[subField.name] || ""}
                        onChange={(e) =>
                          updateItem(idx, subField.name, e.target.value)
                        }
                        rows={2}
                        className="w-full text-xs rounded-md border-2 border-gray-300 dark:border-gray-600
        ...
        <button
          type="button"
          onClick={addItem}
          className="flex items-center justify-center w-full py-2 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-xs text-gray-500 hover:text-indigo-600 hover:border-indigo-500 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-all"
        >
    ...
    <div className="space-y-1.5">
      ...
      {type === "textarea" ? (
        <textarea
          ...
          rows={3}
          className="w-full text-sm rounded-md border-2 border-gray-300 dark:border-gray-600 dark:bg-gray-700 
          dark:text-white focus:ring-indigo-500 focus:border-indigo-500"
        />
      ) : (
        <input
          type={
          ...
          value={value || ""}
          onChange={handleChange}
          className="w-full text-sm rounded-md p-2 border-2 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-indigo-500 focus:border-indigo-500"
        />
      )}
    </div>
  );
...
const FileUploader = ({ value, onChange, accept, compact }) => {
  ...
  return (
    <div className={`relative group ${sizeClass}`}>
      ...
      <div
        className={`w-full h-full border-2 border-dashed rounded-lg flex flex-col items-center justify-center transition-all overflow-hidden bg-gray-50/50 dark:bg-gray-800/30
            ${
              isUploading
