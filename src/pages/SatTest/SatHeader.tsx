...
export const CalculatorWindow: React.FC<CalculatorWindowProps> = ({
  ...
  useEffect(() => {
    ...
    const existing = document.querySelector<HTMLScriptElement>(
      'script[data-desmos-api="1.11"]',
    );
    if (!existing) {
      const script = document.createElement("script");
      script.src =
        "https://www.desmos.com/api/v1.11/calculator.js?apiKey=a13a0bfaa4494af1976eee2b9e5af053";
      script.async = true;
  ...
  const createDesmosCalculator = () => {
    ...
    try {
      ...
      if (
        ...
        typeof calculatorRef.current.setState === "function"
      ) {
        try {
          calculatorRef.current.setState(savedStateRef.current, {
            allowUndo: false,
          });
        } catch (err) {
      ...
      if (
        ...
        typeof calculatorRef.current.observeEvent === "function"
      ) {
  ...
  const handleGetState = () => {
    if (
      calculatorRef.current &&
      typeof calculatorRef.current.getState === "function"
    ) {
      try {
        ...
        alert(
          "State captured (stored in-memory). Use set-state to restore it.",
        );
      } catch (err) {
        console.error(err);
  ...
  const handleClear = () => {
    if (
      calculatorRef.current &&
      typeof calculatorRef.current.setBlank === "function"
    ) {
      try {
          ...
          <div className="space-y-6">
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
              ...
              <p className="text-sm text-slate-700 dark:text-slate-300">
                Our support team is available 24/7 to help with any technical
                issues or questions about the test format.
              </p>
            </div>
        ...
        >
          <form onSubmit={handleReportSubmit} className="space-y-6">
            <p className="text-slate-700 dark:text-slate-300">
              ...
              review your report and get back to you within 24 hours.
            </p>
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800">
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Automatically save your answers every 30 seconds
                  </p>
                </div>
                <button
                  onClick={() =>
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full bg-white transition-transform ${settings.autoSave ? "translate-x-6" : ""}`}
                  />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800">
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full bg-white transition-transform ${settings.reduceAnimations ? "translate-x-6" : ""}`}
                  />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800">
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Play sound when section completes
                  </p>
                </div>
                <button
                  onClick={() =>
                    setSettings((s) => ({
                      ...s,
                      soundOnCompletion: !s.soundOnCompletion,
                    }))
                  }
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full bg-white transition-transform ${settings.soundOnCompletion ? "translate-x-6" : ""}`}
                  />
                </button>
              </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-slate-800 dark:to-slate-800 rounded-xl p-4">
              <p className="text-sm text-slate-700 dark:text-slate-300">
                <strong>Note:</strong> Some settings may require a page refresh
                to take full effect.
              </p>
            </div>
          </div>
