...
export default function GmatTestAttemptPage() {
  ...
  const [selectedSequenceIndex, setSelectedSequenceIndex] = useState<
    number | null
  >(null);
  const [orderSelectionTimer, setOrderSelectionTimer] = useState<number>(120);
  const [isOrderSelectionTimerRunning, setIsOrderSelectionTimerRunning] =
    useState(false);
  const [currentScreen, setCurrentScreen] =
    useState<GmatScreen>("introduction");
    ...
    try {
      ...
      if (meta) {
        ...
        if (screen === "break" && meta.breakExpiresAt) {
          const diffMs = new Date(meta.breakExpiresAt).getTime() - Date.now();
      ...
      toast.error(err?.response?.data?.message || "Failed to start GMAT test");
    } finally {
      setStarting(false);
      setLoading(false);
    }
...
//  }, []);
  ...
  const submitTestAttempt = async (fromTimeUp: boolean = false) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to submit GMAT test");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const computeDiIsAnswered = (
    ...
    switch (subtype) {
      case "multi_source_reasoning": {
        const statements =
          question?.questionDoc?.dataInsights?.multiSource?.statements || [];
      ...
      case "table_analysis": {
        const statements =
          question?.questionDoc?.dataInsights?.tableAnalysis?.statements || [];
      ...
      case "two_part_analysis": {
        const columns =
          question?.questionDoc?.dataInsights?.twoPart?.columns || [];
      ...
      case "graphics_interpretation": {
        const dropdowns =
          question?.questionDoc?.dataInsights?.graphics?.dropdowns || [];
  ...
  const updateDiAnswers = (updater: (prev: DiAnswers) => DiAnswers) => {
    if (!attempt || !currentSection || !currentQuestion) return;
    if (isCompleted) return;
  ...
  const handleConfirmSequence = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to set GMAT order");
    }
  };
  ...
  if (error || !attempt) {
    return (
      <>
        <PageMeta title="GMAT Test" description="GMAT-style test attempt" />
  ...
  const renderBreak = () => (
    <div className="w-full flex flex-col mt-6">
      {/* Main Content Area */}
      <div
        className={`w-full max-w-8xl mx-auto bg-white dark:bg-slate-900 p-6 border-t-4 border-blue-600 dark:border-blue-500 flex-grow`}
      >
        <div className="text-base leading-relaxed text-slate-800 dark:text-slate-200 space-y-4">
          ...
          <p>
            In the official exam, you are allowed one 10-minute break. In this
            practice test, you can either use the full break or skip it and
          ...
          <p>
            Click the{" "}
            <span className="bg-blue-600 text-white px-2 py-1 rounded font-bold dark:bg-blue-700">
              Next →
            </span>{" "}
            button when you are ready to continue to the next section.
          </p>
        </div>
      </div>
      ...
      <div className="w-full flex flex-col mt-6">
        ...
        >
          ...
          <div className="text-base leading-relaxed text-slate-800 dark:text-slate-200 space-y-2">
            ...
            <p>
              You have <strong>{editsRemaining} edit(s)</strong> remaining in
              this section. Click on any question to review and edit your
              answer.
            </p>
            <p>
              You can review any question and edit up to{" "}
              <strong>{editsRemaining} more answers</strong> in this section
              before moving to the next module. Use your edits wisely!
            </p>

            <p className="mt-4">
              Click on any question below to review and edit your answer. When
              you are ready to proceed, click the{" "}
              <span className="bg-blue-600 text-white px-2 py-1 rounded font-bold dark:bg-blue-700">
                Next →
              </span>{" "}
              button.
              ...
              {currentSection.questions.map((q) => {
                ...
                return (
                  <div
                    ...
                    className={`group flex items-center justify-between p-2 rounded-xl border-2 cursor-pointer transition-all duration-200 ${
                      isBookmarked
      ...
      <div className="flex-1 flex items-center justify-center text-center px-6">
        <div className="max-w-4xl">
          ...
          <p className="text-base text-slate-600 mb-8">
            Please note: the pause button is intended for short pauses or
            breaks. If you expect to be away for more than 20 minutes, we
            suggest you save your exam for later. Otherwise, your exam will
