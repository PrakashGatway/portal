...
export default function TestTemplateManagementPage() {
  ...
  const [questionModalSectionIndex, setQuestionModalSectionIndex] = useState<
    number | null
  >(null);
  ...
  const [questionModalSelectedIds, setQuestionModalSelectedIds] = useState<
    string[]
  >([]);
  ...
  const allQuestionsSelected =
    ...
    questionModalList.every((q) => questionModalSelectedIds.includes(q._id));

  const someQuestionsSelected =
    questionModalList.some((q) => questionModalSelectedIds.includes(q._id)) &&
    !allQuestionsSelected;

  const handleToggleSelectAllQuestions = () => {
    const currentIds = questionModalList.map((q) => q._id);

    const allCurrentSelected = currentIds.every((id) =>
      questionModalSelectedIds.includes(id),
    );

    if (allCurrentSelected) {
      // Remove only questions from the CURRENT search result.
      setQuestionModalSelectedIds((prev) =>
        prev.filter((id) => !currentIds.includes(id)),
      );

      setSelectedQuestions((prev) =>
        prev.filter((q) => !currentIds.includes(q._id)),
      );
    } else {
      // Add current search results without removing previous selections.
      setQuestionModalSelectedIds((prev) => [
      ...
      setSelectedQuestions((prev) => {
        ...
        const newQuestions = questionModalList.filter(
  ...
  const fetchTests = async () => {
    try {
      ...
      if (filters.difficultyLabel !== "all")
        params.difficultyLabel = filters.difficultyLabel;
  ...
  // Question Modal
  const openQuestionModalForSection = async (
    sectionIndex: number,
    ...
    try {
      ...
      if (initializeSelection) {
        const currentSelected: string[] =
          watch(`sections.${sectionIndex}.questionIds` as const) || [];

        setQuestionModalSelectedIds(currentSelected);
        if (currentSelected.length > 0) {
          try {
            const selectedRes = await api.get("/mcu/questions", {
              params: {
                examId,
                sectionId,
                ids: currentSelected,
                limit: currentSelected.length,
              },
            });

            if (selectedRes.data?.success) {
              const selectedList = (selectedRes.data.data ||
                selectedRes.data?.data?.data ||
                []) as any[];
      ...
      if (questionModalSearch.trim()) {
        params.search = questionModalSearch.trim();
      }
  ...
  const refreshQuestionList = async () => {
    ...
    await openQuestionModalForSection(questionModalSectionIndex, false);
  };
  ...
  const toggleQuestionSelection = (question: QuestionSummary) => {
    const isSelected = questionModalSelectedIds.includes(question._id);
    ...
    setQuestionModalSelectedIds((prev) => [...prev, question._id]);
  ...
  const openEditDrawer = async (id: string) => {
    try {
      ...
      const mappedSections =
        test.sections && test.sections.length
          ? test.sections.map((s: any, index: number) => ({
              ...
              questionCount:
                s.questionCount || s.randomConfig?.questionCount || 0,
      ...
      reset({
        ...
        quizAllowedTypesInput: Array.isArray(
          test.quizConfig?.allowedQuestionTypes,
        )
  ...
  const onSubmit = async (values: TestTemplateFormValues) => {
    try {
      ...
      if (values.testType !== "quiz") {
        ...
        const invalidSection = values.sections.find((s) => !s.sectionId);
        if (invalidSection) {
      ...
      if (values.testType !== "quiz") {
        sectionsPayload = values.sections.map((s, index) => {
          const randomTypes = s.randomQuestionTypes
            ? s.randomQuestionTypes
                .split(",")
                .map((x) => x.trim())
                .filter(Boolean)
            : [];
          const randomDiffs = s.randomDifficulties
            ? s.randomDifficulties
                .split(",")
                .map((x) => x.trim())
                .filter(Boolean)
            : [];
  ...
  const handleDelete = async (test: TestTemplateListItem) => {
    if (
      !window.confirm(
        `Delete test "${test.title}"? This action cannot be undone.`,
      )
    )
      return;
    try {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selectedTests.map((id) => api.delete(`/mcu/test/${id}`)),
      );
  ...
  const priceLabel = (t: TestTemplateListItem) => {
    if (t.isFree)
      return { label: "Free", color: "text-emerald-600 dark:text-emerald-400" };
    if (t.seriesOnly && !t.isSellable)
      return {
        label: "Series Only",
            ...
            {tests.map((t) => {
              ...
              return (
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                  className={`group relative border rounded-2xl bg-white p-4 transition-all hover:shadow-lg dark:bg-gray-900 cursor-pointer ${
                ...
                >
                  ...
                  <div className="mb-2 flex flex-wrap gap-2">
                    <span
                      className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium ${getDifficultyColor(t.difficultyLabel)}`}
                    >
                      {t.difficultyLabel}
                    </span>
                    <span className="inline-flex items-center gap-1 rounded-full border border-gray-200 bg-gray-50 px-2 py-0.5 text-xs font-medium text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                      ...
                      {t.sectionCount || 1}{" "}
                      {t.sectionCount === 1 ? "Section" : "Sections"}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between dark:border-gray-800">
                    ...
                    <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      <button
                        ...
                        onClick={() => {
                          if (t.exam?.name.toLowerCase().includes("gmat")) {
                            navigate(`/gmat/tests/${t._id}`, { replace: true });
                          } else if (
                            t.exam?.name.toLowerCase().includes("pte")
                          ) {
                            navigate(`/pte/tests/${t._id}`, { replace: true });
                          } else if (
                            t.exam?.name.toLowerCase().includes("gre")
                          ) {
                            navigate(`/gre/tests/${t._id}`, { replace: true });
                          } else
                            navigate(`/mcq/tests/${t._id}`, { replace: true });
                        }}
                      >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 text-left dark:border-gray-800">
                  ...
                  <th className="p-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                    Test Name
                  </th>
                  <th className="p-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                    Exam
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                          {TEST_TYPE_OPTIONS.find((x) => x.value === t.testType)
                            ?.label || t.testType}
                        </span>
                      </td>
                      <td className="p-4">
                        <span
                          className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${getDifficultyColor(t.difficultyLabel)}`}
                        >
                          {t.difficultyLabel}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-1">
                          <IndianRupee
                            className={`h-3.5 w-3.5 ${price.color}`}
                          />
                          <span
                            className={`text-sm font-medium ${price.color}`}
                          >
                            {price.label}
                          </span>
                        </div>
                ...
                <div>
                  ...
                  <p className=" text-xs text-gray-500 dark:text-gray-400">
                    {editingId
                      ...
                      : "Configure your new test template"}
                  </p>
                </div>
                <button
                  onClick={saving ? undefined : closeDrawer}
                      ...
                      <div className="rounded-2xl border border-gray-200 p-5 dark:border-gray-800">
                        ...
                        {sectionFields.length === 0 ? (
                          <div className="rounded-xl border-2 border-dashed border-gray-200 p-8 text-center dark:border-gray-700">
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                              No sections added yet. Click "Add Section" to
                              begin.
                            </p>
                          </div>
                        ) : (
                                  ...
                                  {selectionMode === "fixed" ? (
                                          ...
                                          <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                                            {
                                              (
                                                watch(
                                                  `sections.${index}.questionIds` as const,
                                                ) || []
                                              ).length
                                            }{" "}
                                            questions selected
                                    ...
                                    <div className="mt-4 space-y-3 rounded-lg border border-dashed border-gray-300 bg-white p-3 dark:border-gray-600 dark:bg-gray-800">
                                      ...
                                      <p className="text-xs text-gray-400">
                                        Use comma-separated values. Leave empty
                                        to include all.
                                      </p>
                                    </div>
                                  )}
                      ...
                      <div className="mb-4 flex flex-wrap gap-4">
                        <label className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm dark:border-gray-700 dark:bg-gray-800">
                          <input
                            ...
                            onChange={(e) =>
                              setValue("pricingIsSellable", e.target.checked)
                            }
                            className="rounded"
                          />
                        ...
                        <label className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm dark:border-gray-700 dark:bg-gray-800">
                          <input
                            ...
                            onChange={(e) =>
                              setValue("pricingSeriesOnly", e.target.checked)
                            }
                            className="rounded"
                          />
                          <span className="text-gray-700 dark:text-gray-200">
              ...
              <div className="p-5">
                ...
                <div className="flex gap-2">
                  {selectedQuestions.length > 0 && (
                    <div className="max-w-2/5 rounded-xl  border border-blue-200 bg-blue-50/50 p-3 pr-1 dark:border-blue-900/50 dark:bg-blue-900/10">
                      <div className="mb-3 flex items-center justify-between">
                        <div>
                      ...
                      <div className="max-h-[43vh] space-y-1 overflow-y-auto">
                        {selectedQuestions.map((q, index) => (
                          <div
                            key={q._id}
                            className="flex items-start gap-3 rounded-lg border border-blue-100 bg-white p-2 dark:border-blue-900/40 dark:bg-gray-800"
                          >
                            <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-semibold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                              {index + 1}
                            </div>

                            <div className="min-w-0 flex-1">
                              <p
                                className="line-clamp-2 text-sm text-gray-800 dark:text-gray-100"
                              ...
                              <div className="mt-1 flex items-center gap-2">
                                ...
                                <span
                                  className={`rounded-full px-2 py-0.5 text-[11px] ${getDifficultyColor(
                                    q.difficulty,
                                  )}`}
                                >
                                  {q.difficulty}
                      ...
                      {questionModalLoading ? (
                        ...
                        <div className="divide-y divide-gray-100 dark:divide-gray-800">
                          {questionModalList.map((q) => {
                            const selected = questionModalSelectedIds.includes(
                              q._id,
                            );
                            return (
                              <div
                                key={q._id}
                                className={`cursor-pointer px-4 py-3 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 ${
                                  selected
                                    ? "bg-blue-50/50 dark:bg-blue-900/10"
                                    : ""
                                }`}
                                ...
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="mt-1 flex items-center gap-2">
                                      ...
                                      <span
                                        className={`rounded-full px-2 py-0.5 text-xs ${getDifficultyColor(q.difficulty)}`}
                                      >
                                        {q.difficulty}
                                      </span>
                                    </div>
