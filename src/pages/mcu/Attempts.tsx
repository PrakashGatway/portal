...
interface AttemptQuestion {
  question: string; // ObjectId string
  order: number;
...
interface StartAttemptResponse {
  _id: string;
  // plus other fields we don't care now
}
...
export default function SatTestAttemptPage() {
  ...
  // for tracking time spent on current question
  ...
  // 1️⃣ Start attempt (or resume) then fetch full attempt with question docs
  const startAttempt = useCallback(async () => {
    ...
    try {
      ...
      // Fetch full attempt with questionDoc populated
      ...
      // const totalDurationMinutes = loaded.totalDurationMinutes || 60;
      // const used = loaded.totalTimeUsedSeconds || 0;
      // const totalSeconds = totalDurationMinutes * 60;
      // const left = Math.max(0, totalSeconds - used);
      ...
      // 🔍 find first section that has questions
      const firstSectionWithQuestions = loaded.sections.findIndex(
        (s) => s.questions && s.questions.length > 0
      );

      if (firstSectionWithQuestions === -1) {
        setError("This test has no questions configured.");
        setLoading(false);
        return;
      }

      setActiveSectionIndex(firstSectionWithQuestions);
      setActiveQuestionIndex(0);

      // ⏱️ section-wise timer: use current section duration
      const firstSec = loaded.sections[firstSectionWithQuestions];
      const secDurationMinutes =
        firstSec.durationMinutes || loaded.totalDurationMinutes || 60;
      const secDurationSeconds = secDurationMinutes * 60;

      // how much time already spent in this section (sum of question times)
      const usedInSection = firstSec.questions.reduce(
      ...
      toast.error(err.response?.data?.message || "Failed to start SAT test");
    } finally {
      setStarting(false);
      setLoading(false);
  ...
  useEffect(() => {
    ...
    setTimerSecondsLeft(left);
    setTimerRunning(
      attempt.status === "in_progress" && left > 0 && !isCompleted
    );
  }, [attempt, activeSectionIndex, isCompleted]);


  // 2️⃣ Timer: SAT-style single countdown
  useEffect(() => {
  ...
  }, [attempt, timerRunning, timerSecondsLeft, activeSectionIndex, activeQuestionIndex, isCompleted]);
  ...
  useEffect(() => {
    ...
    if (timerSecondsLeft === 0) {
      const isLastSection = activeSectionIndex >= attempt.sections.length - 1;

      if (isLastSection) {
        setTimerRunning(false);
        toast.info("Time is up for the last section. Submitting your test...");
        submitTestAttempt(true);
      } else {
        toast.info("Time is up for this section. Moving to the next section.");
        setTimerRunning(false);
        // this will save current, switch, and the section-change effect will re-init timer
        changeSection(activeSectionIndex + 1);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [timerSecondsLeft, timerRunning, attempt, activeSectionIndex]);


  // 3️⃣ Helpers for reading current section & question
  ...
  // 4️⃣ Save current question progress before navigation / submitting
  const saveCurrentQuestionProgress = useCallback(
    async (silent: boolean = true) => {
      ...
      try {
        ...
        await api.patch(`/mcu/attempts/${attempt._id}/save-progress`, {
          updates,
  ...
  const handleOptionClick = (optionIndex: number) => {
    ...
    setAttempt((prev) => {
      ...
      // SAT: single correct answer per question → single index
  ...
  // 6️⃣ Answer handler for text/numeric
  ...
  // 8️⃣ Navigation between questions
  ...
  const goNextQuestion = async () => {
    if (!currentSection) return;
    if (activeQuestionIndex >= currentSection.questions.length - 1) return;
    await goToQuestion(activeQuestionIndex + 1);
  };

  const changeSection = async (sectionIndex: number) => {
    if (!attempt) return;
    if (sectionIndex < 0 || sectionIndex >= attempt.sections.length) return;
  ...
  const submitTestAttempt = async (fromTimeUp: boolean = false) => {
    ...
    if (!fromTimeUp) {
      const confirmed = window.confirm(
        "Are you sure you want to submit your SAT test? You won't be able to change your answers afterwards."
      );
      if (!confirmed) return;
    }
    ...
    try {
      setSubmitting(true);
      setTimerRunning(false);
      // Save current question one last time
      await saveCurrentQuestionProgress(true);

      const res = await api.post(`/mcu/attempts/${attempt._id}/submit`);
      if (!res.data?.success) {
        throw new Error(res.data?.message || "Failed to submit test");
      }
  ...
  if (loading || starting) {
    return (
      <>
        <PageMeta title="SAT Test" description="SAT style test attempt" />
        ...
        <div className="flex min-h-[60vh] items-center justify-center">
          <div className="max-w-md rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-700 shadow-sm dark:border-red-800 dark:bg-red-900/20 dark:text-red-200">
            <div className="mb-2 flex items-center gap-2">
              ...
              <h2 className="font-semibold">Unable to load SAT test</h2>
            </div>
            <p className="mb-3">
  ...
  const isMCQ = !!(qDoc && qDoc.options && qDoc.options.length);

  return (
    <>
      <PageMeta
        title={attempt.testTemplate.title || "SAT Test"}
        description="SAT style test attempt"
      />
      <div className="relative min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950">
        {/* Top bar */}
        <div className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/90">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden flex-col text-[10px] text-slate-500 dark:text-slate-400 sm:flex">
                ...
                <span>Section {activeSectionIndex + 1} of {attempt.sections.length}</span>
              </div>
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-3 py-3 lg:flex-row">
          ...
          <div className="flex-1 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            {/* Section tabs (SAT style) */}
            <div className="mb-3 flex flex-wrap gap-2 text-xs">
              {attempt.sections.map((sec, idx) => {
                const isActive = idx === activeSectionIndex;
                const answeredCount = sec.questions.filter((q) => q.isAnswered)
                  .length;
                return (
                  <button
                    key={idx}
                    onClick={() => changeSection(idx)}
                    className={`flex items-center gap-2 rounded-full border px-3 py-1 transition-all ${isActive
                      ? "border-blue-600 bg-blue-50 text-blue-700 shadow-sm dark:border-blue-400 dark:bg-blue-950/60 dark:text-blue-200"
                      : "border-slate-200 bg-slate-50 text-slate-600 hover:border-blue-400 hover:text-blue-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-blue-400"
                      }`}
                  >
                    ...
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">
                      {sec.name || "SAT Section"} • {answeredCount}/
                      {sec.questions.length} answered
                    </span>
                  </button>
                );
              })}
            ...
            {/* Stimulus (passage / image / etc.) */}
            {qDoc?.stimulus && (
              <div className="mb-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                {/* You can render as HTML if needed */}
            ...
            <div className="space-y-3">
              {isMCQ ? (
                  ...
                  {qDoc!.options!.map((opt, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-start gap-2 rounded-xl border px-3 py-2 text-left text-sm transition ${selected
                          ? "border-blue-600 bg-blue-50 text-blue-900 shadow-sm dark:border-blue-400 dark:bg-blue-950/60 dark:text-blue-100"
                          : "border-slate-200 bg-slate-50 text-slate-800 hover:border-blue-400 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-blue-400"
                          }`}
                      >
                        <div
                          className={`mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full border text-xs font-semibold ${selected
                ...
                <div className="space-y-2">
                  ...
                  <input
                    type="text"
                    className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50"
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 pt-3 text-xs dark:border-slate-800">
                <div className="flex flex-wrap items-center gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className={`flex items-center gap-1 rounded-full px-3 py-1 ${currentQuestion.markedForReview
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      activeQuestionIndex >=
                      totalQuestionsForCurrentSection - 1
                    }
                  >
                    Next
                ...
                {paletteStatus.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex h-8 w-8 items-center justify-center rounded-full border text-[11px] font-semibold transition ${isCurrent
                        ? "border-blue-600 shadow-sm shadow-blue-500/40"
              ...
              <div className="space-y-1 text-[11px] text-slate-600 dark:text-slate-300">
                ...
                {!isCompleted && (
                  <div className="mt-1 text-[10px] text-slate-500 dark:text-slate-400">
                    Your detailed SAT-like score report will be available after
                    submission.
                  </div>
                )}
              </div>
